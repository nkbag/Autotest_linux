...
FOLDER_TST = "/home/userok/tst"
FOLDER_OUT = "/home/userok/out"
FOLDER_folder1 = "/home/userok/folder1"
...
def test_1(start_time, statistics):
    res = []
...
def test_step1(clear_dir, get_dir, start_time, statistics):
    res = []
    res.append(ssh_checkout("0.0.0.0", "userok", "2222", f"cd {FOLDER_TST}; 7z a {FOLDER_OUT}/arx2", "Everything is Ok"))
    res.append(ssh_checkout("0.0.0.0", "userok", "2222", f"ls {FOLDER_OUT}", "arx2.7z"))
...
def test_step2(clear_dir, get_dir, make_files, start_time, statistics):
    res = []
    res.append(ssh_checkout("0.0.0.0", "userok", "2222", f"cd {FOLDER_TST}; "
                                                         f"7z a {FOLDER_OUT}/arx2", "Everything is Ok"))
    res.append(ssh_checkout("0.0.0.0", "userok", "2222", f"cd {FOLDER_OUT}; "
                                                         f"7z e arx2.7z -o{FOLDER_folder1} -y", "Everything is Ok"))
    for i in make_files:
        res.append(ssh_checkout("0.0.0.0", "userok", "2222", f"ls {FOLDER_folder1}", i))
...
def test_step3(start_time, statistics):
    ...
    assert ssh_checkout("0.0.0.0", "userok", "2222", f"cd {FOLDER_OUT}; "
...
def test_step5(get_list, start_time, statistics):
    ...
    assert ssh_checkout("0.0.0.0", "userok", "2222",
