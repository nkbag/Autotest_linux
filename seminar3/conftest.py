...
def make_files():
    ...
    for i in range(data["count"]):
        ...
        if checkout(f"cd {FOLDER_TST}; dd if=/dev/urandom of={filename} bs={data['size']} count={data['count']} iflag=fullblock", ""):
...
@pytest.fixture()
def statistics(start_time):
    name = "stat.txt"
    with open(name, "a", encoding="utf-8") as f:
        f.write(''.join(get_out(f"echo 'время: {start_time}, количество файлов: {count}, размер файла: {size}\n'; cat /proc/loadavg >> {name}")[0]))